...
int main(int argc, char* argv[])
{
	...
	while (loadfile >> pathname) { // load filenames
		...
		//-----------------------------------------
			...
			splitpath.close();
			pathFromPoints(path, temp, false);
		...
		// initialize weights
		//vector<vector<int>> weights (Weights, Weights +sizeof(Weights)/sizeof(Weights[0]));

		for (int pylon = 0; pylon < (sizeof(Weights) / sizeof(Weights[0])); pylon++) {

			// vector of weights
			vector<float> weights(Weights[pylon], Weights[pylon] + sizeof(Weights[pylon]) / sizeof(Weights[pylon][0]));
			...
			for (unsigned int i = 0; i < candidatesStart.size(); i++) {
				...
				for (unsigned int j = 0; j < candidatesEnd.size(); j++) {
					costmatrix(i, j) = pathMisMatch(candidatesStart[i], candidatesEnd[j], weights);
				}
			}
			...
			for (unsigned int i = 0; i < candidatesStart.size(); i++)
				for (unsigned int j = 0; j < candidatesEnd.size(); j++)
					if (costmatrix(i, j) == 0) {
						matchtemp[i] = j;
						strengthtemp[i] = pathMatch(candidatesStart[i], candidatesEnd[j], weights);
					}
			//**********************
